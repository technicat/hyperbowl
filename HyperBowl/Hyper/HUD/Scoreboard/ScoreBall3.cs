...
public class ScoreBall3 : MonoBehaviour {
		...
		private Hashtable htend;
			...
			void Play () {
				...
				if (frame == Bowl.frame && Bowl.roll == 2 && Bowl.currentplayer == hyperplayer) {
					PlayStart();
				} else {
					ActivateDigit();
				}
			...
			void ActivateDigit() {
				int score = Bowl.GetCurrentScores(hyperplayer)[frame].ball3;
				if (score>-1) {
